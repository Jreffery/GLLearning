...
import cc.appweb.gllearning.databinding.ActivityApi1TextureviewBinding
...
class API1TextureViewActivity : AppCompatActivity(), View.OnClickListener {
    ...
    companion object {
        const val TAG = "TAG_API1TextureView"
        const val CAMERA_PERMISSION_RESULT = 1
    }

    private lateinit var mActivityBinding: ActivityApi1TextureviewBinding
    ...
    // 保存resume/pause生命周期下是否恢复预览
    private var mKeepOpening: Boolean = false

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_api1_textureview)

        mActivityBinding = ActivityApi1TextureviewBinding.bind(findViewById(R.id.api1_textureview_container))
...
//                    Log.i(TAG, "onPreviewFrame")
                ...
                if (mActivityBinding.preview.isAvailable) {
                    itt.setPreviewTexture(mActivityBinding.preview.surfaceTexture)
                    itt.startPreview()
                }
            }
        }
    ...
    private fun closeCamera() {
        Log.i(API1SurfaceViewActivity.TAG, "closeCamera")
        mCamera?.let {
